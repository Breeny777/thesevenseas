...
SPOTIFY_REDIRECT_URI = "http://localhost:8888/callback"  # doesn't need to exist on server
...
def lidarr_add_artist(artist_obj):
    payload = {
...
def lidarr_trigger_album_search(album_id):
...
def main():
    ...
    # 3. Build Plex "have" set
    print("\nBuilding Plex library index (section 6)...")
    plex_have = get_plex_tracks()
    print(f"Plex has {len(plex_have)} tracks indexed.\n")
    ...
    for pl in chosen:
        ...
        for item in tracks:
            ...
            if album_key not in seen_albums:
                ...
                if album_obj:
                    lidarr_trigger_album_search(album_obj["id"])
